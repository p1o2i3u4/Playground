...
def listVideoCategories(url):
    try:
        url = "https://www.tvnamu2.me/category/%EB%93%9C%EB%9D%BC%EB%A7%88-1/1/1.html"
        ...
        for title, url, thumbnail in items:
            addDir(title, url, 'videoCategoryContent', thumbnail)
...
def listVideosInCategory(url):
    try:
        ...
        for title, url, thumbnail in items:
            addDir(title, url, 'listepisodes', thumbnail)
...
def listvideourl(url):
    try:
        ...
        soup2=soup.find("input", {"id":"link_gogovid"})['value']
